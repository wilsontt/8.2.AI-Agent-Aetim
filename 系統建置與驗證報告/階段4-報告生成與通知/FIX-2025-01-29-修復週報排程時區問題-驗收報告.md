...
1. **APScheduler 硬編碼為 UTC 時區**
   - **檔案**：`reporting_notification/application/services/report_schedule_service.py`
   - **位置**：第 63-68 行
   - **問題**：`timezone='UTC'` 硬編碼，未從環境變數讀取

2. **ReportSchedule 領域模型缺少 timezone 欄位**
   - **檔案**：`reporting_notification/domain/aggregates/report_schedule.py`
   - **問題**：使用者在前端設定的時區資訊無法儲存
